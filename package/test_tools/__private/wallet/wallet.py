...
class Wallet(UserHandleImplementation, ScopedObject):
    ...
    def __init__(
        ...
        *,
        ...
        self.__retrived_authorities: dict[bytes, wax.python_authorities] | None = None
    ...
    @property
    def retrived_authorities(self) -> dict[bytes, wax.python_authorities]:
        assert self.__retrived_authorities is not None
        return self.__retrived_authorities

    def __get_chain_id(self) -> str | Hex:
        ...
        if self.__chain_id != "default":
            ...
            return chain_id_hash_object.hexdigest()
    ...
    def complex_transaction_sign(self, transaction: SimpleTransaction) -> SimpleTransaction:
        ...
        sign_keys = self.import_required_keys(transaction_bytes)
        signed_transaction = self.sign_transaction(transaction, sig_digest, sign_keys)
        reduced_sign_keys = self.reduce_signatures(signed_transaction, sign_keys)
    ...
    def reduce_signatures(
        self, transaction: SimpleTransaction, keys_to_sign_with: list[str] | list[Any]
    ) -> list[str] | list[Any]:
        def retrieve_witness_key(wittnes_name: bytes) -> bytes:
            assert self.connected_node is not None, "Node not exist"
            get_witness = self.connected_node.api.wallet_bridge.get_witness(wittnes_name.decode())
            assert get_witness is not None
            return get_witness.signing_key.encode()

        transaction_bytes = transaction.json().encode()
        available_keys = [key.encode() for key in keys_to_sign_with]
        authorities_map = self.retrived_authorities
        chain_id = self.__get_chain_id()
        python_minimize_required_signatures_data = wax.python_minimize_required_signatures_data(
            chain_id=chain_id.encode(),
        ...
        minimized_signatures = wax.minimize_required_signatures(
            transaction_bytes, python_minimize_required_signatures_data
        )
        return [key.decode() for key in minimized_signatures]
    ...
    def import_required_keys(self, transaction: bytes) -> list[str] | list[Any]:
        ...
        def retrieve_authorities(account_names: list[bytes]) -> dict[bytes, wax.python_authorities]:
            ...
            self.__retrived_authorities = {acc.name.encode(): to_python_authorities(acc) for acc in accounts}
            return self.__retrived_authorities

...
from test_tools.__private.beekeeper_wallet import SingleTransactionContext, BeekeeperWallet
...
class BeekeeperWalletHandle(Handle):
    DEFAULT_PASSWORD = BeekeeperWallet.DEFAULT_PASSWORD
    ...
    def __init__(
        ...
        super().__init__(
            implementation=BeekeeperWallet(
                attach_to=attach_to,
            )
        )
    ...
    @property
    def __implementation(self) -> BeekeeperWallet:
        return get_implementation(self, BeekeeperWallet)
    ...
    def run(
        self,
        *,
        timeout: float = BeekeeperWallet.DEFAULT_RUN_TIMEOUT,

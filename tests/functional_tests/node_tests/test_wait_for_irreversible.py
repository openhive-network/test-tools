...
def test_raising_block_wait_timeout_error(include_number_param: bool, node: tt.InitNode):
    # ARRANGE
    blocks_to_wait: Final[int] = 1
    assert blocks_to_wait < 21, "blocks_to_wait should be lower than 21, else irreversible block number increases"

    last_block_number = node.get_last_block_number()

    # ACT & ASSERT
    with pytest.raises(tt.exceptions.BlockWaitTimeoutError):

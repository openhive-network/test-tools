...
def test_if_snapshot_is_dumped_and_node_continues_to_run(node: tt.InitNode):
    node.dump_snapshot()
    assert node.is_running()


def test_if_snapshot_is_dumped_and_node_is_closed(node: tt.InitNode):
    node.dump_snapshot(close=True)
    assert not node.is_running()


def test_if_operation_could_be_created_right_after_dumping_snapshot(node: tt.InitNode):
    # ARRANGE

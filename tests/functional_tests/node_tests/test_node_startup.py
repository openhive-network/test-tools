...
def test_init_node_startup(node: tt.InitNode):
...
def test_loading_own_snapshot(node: tt.InitNode):
...
def test_loading_snapshot_from_other_node(node: tt.InitNode):
    make_transaction_for_test(node)
    generate_blocks(node, 100)  # To make sure, that block with test operation will be stored in block log
...
def test_loading_snapshot_from_custom_path(node: tt.InitNode):
    make_transaction_for_test(node)
    generate_blocks(node, 100)  # To make sure, that block with test operation will be stored in block log
    snapshot = node.dump_snapshot()
...
def test_replay_from_other_node_block_log(node: tt.InitNode):
    make_transaction_for_test(node)
    generate_blocks(node, 100)  # To make sure, that block with test operation will be stored in block log
    node.close()
...
def test_replay_until_specified_block(node: tt.InitNode):
...
def test_replay_from_external_block_log(node: tt.InitNode):
    generate_blocks(node, 100)
    node.close()

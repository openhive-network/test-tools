...
def test_loading_own_snapshot(node: tt.InitNode) -> None:
    ...
    node.run(time_control=tt.StartTimeControl(start_time="head_block_time"), load_snapshot_from=snapshot, wait_for_live=False)
...
def test_replay_from_other_node_block_log(node: tt.InitNode) -> None:
    ...
    replaying_node.run(time_control=tt.StartTimeControl(start_time="head_block_time"), replay_from=node.block_log, wait_for_live=False)
...
def test_replay_until_specified_block(node: tt.InitNode) -> None:
    ...
    replaying_node = tt.ApiNode()
    replaying_node.run(time_control=tt.StartTimeControl(start_time="head_block_time"), replay_from=node.block_log, stop_at_block=50, wait_for_live=False)
...
def test_replay_from_external_block_log(node: tt.InitNode) -> None:
    ...
    replaying_node = tt.ApiNode()
    replaying_node.run(time_control=tt.StartTimeControl(start_time="head_block_time"), replay_from=external_block_log.path, stop_at_block=50, wait_for_live=False)
    assert replaying_node.get_last_block_number() == expected_number_of_blocks

...
def test_correct_type_of_arguments_in_percentage_range_argument(percentage_range):
    tt.Asset.Range(tt.Asset.Test(1), tolerance=percentage_range)


@pytest.mark.parametrize("percentage_range,", [None, False, {}, [], (), set(), -3, "string", range(0, 1)])
...
def test_simultaneously_using_upper_limit_and_percentage_range_arguments():
    with pytest.raises(TypeError):
        tt.Asset.Range(tt.Asset.Test(1), tt.Asset.Test(2), tolerance=5)
...
def test_banned_way_to_creating_a_asset_range_object_without_specifying_arguments():
    with pytest.raises(TypeError):
        tt.Asset.Range()  # pylint: disable=no-value-for-parameter
